...
async function getTxData() {
  ...
  const decoded = _interface.parseTransaction({data: tx.value.data, value: tx.value.value});
  console.log(decoded);

  const data = tx.value.data;
...
<template>
  <div>
    <div class="mx-auto max-w-7xl">
      ...
      <RouterLink :to="{name: 'address', params: {'address': '0xe84d601e5d945031129a83e5602be0cc7f182cf3'}}">
        From: {{tx.from}}
      </RouterLink>

      <RouterLink :to="{name: 'address', params: {'address': tx.to}}">
        To: {{tx.to}}
      </RouterLink>
    </div>
  </div>
